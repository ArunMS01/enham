...
<section class="inner-section single-banner" style="background: url(images/banner-image.jpg) no-repeat center;">
    ...
    if (isset($_GET['url'])) {
        ...
        $supsubcategory = new CategoryController;
        $getallsupsub = $supsubcategory->supersubcategoryurlname($url);
        if ($getallsupsub) {
            foreach ($getallsupsub as $allsupsub) {
    ?>

                <div class="container">
                    <h2><?php echo $allsupsub['supsub_name'] ?></h2>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="/">Home</a> / </li>
                        <li class="breadcrumb-item c_white" aria-current="page"><a href="<?php echo SITE_URL?>category/<?php echo $allsupsub['category_url'] ?>"><?php echo $allsupsub['category_name'] ?></a> / </li>
                        <li class="breadcrumb-item c_white" aria-current="page"><a href="<?php echo SITE_URL?>subcategory/<?php echo $allsupsub['subcategory_url'] ?>"><?php echo $allsupsub['subcat_name'] ?></a> / </li>
                        <li class="breadcrumb-item c_white" aria-current="page"><?php echo $allsupsub['supsub_name'] ?> </li>
    ...
    <div class="container">
        ...
        <div class="row row-cols-2 row-cols-md-3 row-cols-lg-3 row-cols-xl-5" id="load_more_product">
            ...
            if (isset($_GET['url'])) {
                ...
                $result = $category->getsupersubCategorybyurl($url);
                if ($result) {
                    $product = new ProductController;
                    $getProduct = $product->getsupersubcategoryProducts($result);
                    if ($getProduct) {
                                ...
                                <div class="product-card">
                                    <div class="product-media">
                                        ...
                                        <?php if (!isset($_SESSION['authenticated'])) { ?>
                                            <button class="product-wish wish product-wishlist-modal"><i class="fas fa-heart"></i></button>
                                            ...
                                            ?>
                                                <div class="product-action-1">
                                                    ...
                                                    <div class="product-action hjj">
                                                        <a class="btn btn-inline" href="<?php echo SITE_URL?>cart.php">Go to cart</a>
                                                        <!-- <button data-productid="<?php echo $category_products['id'] ?>" class="action-minus add_to_cart_btn" title="Quantity Minus"><i class="icofont-minus"></i></button> -->
                                                        <input type="hidden" value="1" max="<?php echo $category_products['quantity'] ?>" title="Quantity Number" name="qtybutton" class="cart-quantity action-input cart-plus-minus-box">
                                                        <!-- <button data-productid="<?php echo $category_products['id'] ?>" class="action-plus add_to_cart_btn" title="Quantity Plus"><i class="icofont-plus"></i></button> -->
                                                    </div>
                                                </div>
                                            <?php
...
<script>
    ...
    function loadproducts(last_product_id) {
        //  alert(sortValue);
        let supsubcategoryid = "<?php echo $result; ?>";
        ...
        $.ajax({
            ...
            data: {
                ...
                supsubcategoryid: supsubcategoryid,
                sortValue: sortValue,
                tags: tags,
                action: 'load-supsubproducts'
            },
            dataType: "text",
    ...
    function loadproductByfilters(last_product_id) {
        ...
        $.ajax({
            ...
            data: {
                ...
                sortValue: sortValue,
                tags: tags,
                action: 'load-supsubfilterproducts'
            },
            dataType: "text",
            success: function(data) {
                if (data != '') {
    ...
    $('#show-more-btn').on('click', function() {
        var last_product_id = '';
        last_product_id = $(this).data("productid");
        loadproducts(last_product_id);
    });

    $('#price_sorting').on('change', function() {
        var last_product_id = '';
        last_product_id = $('#show-more-btn').data("productid");
