...
if(isset($_POST['checkout-login'])){
    ...
    $result = $auth->loginUser($email,$password);
    if($result){
        redirect("Login Succesfully", "checkout.php"); 
    }
    else{
        redirect("Invalid email or password", "cart.php");  
    }
}
...
if(isset($_POST['wishlist-login'])){
    ...
    $result = $auth->loginUser($email,$password);
    if($result){
        redirectback("Login Succesfully"); 
    }
    else{
    ...
    if($check_password){
        ...
        if($check_user){
            ...
            if($registerUser){
                ...
                <a href="https://enham.in/activate-user.php?token='.$token.'&email='.$email.'">Click here to activate your account</a>
                ';
...
if(isset($_POST['forget-btn'])){
    ...
    if($result){
          $subject ="Reset Password";
                ...
                <a href="https://enham.in/password-change.php?token='.$token.'&email='.$email.'">Click here to reset password</a>
                ';
                $mailsend = $sendVerificationmail->mailsend($content, $email, $subject);
